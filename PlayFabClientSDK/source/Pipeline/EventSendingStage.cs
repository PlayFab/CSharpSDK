...
{/*
    /// <summary>
    /// Strong-type implementation of event arguments for RaiseStageNotificationEvent C# event that is relevant for this stage.
    /// </summary>
    public class BatchSendOutcomeEventArgs : PipelineStageNotificationEventArgs<TitleEventBatch, HttpApiResponse<Event.WriteEventsResult>>
    {
        public BatchSendOutcomeEventArgs(TitleEventBatch batch, HttpApiResponse<Event.WriteEventsResult> sendResult) : base(batch, sendResult)
        {
        }
    }*/
    ...
    internal class EventSendingStage : PipelineStageBase<TitleEventBatch, PlayFabResult<WriteEventsResponse>>
    {
        ...
        protected override void OnNextInputItem(TitleEventBatch batch)
        {
            ...
            try
            {
                ...
                this.FulfillPromises(batch.Events, apiTask.Result);
                //this.OnRaiseStageNotificationEvent(new BatchSendOutcomeEventArgs(batch, writeEventsTask.Result));
            }
            catch (Exception e)
            {
                // Cancel result promises that will never be fulfilled
